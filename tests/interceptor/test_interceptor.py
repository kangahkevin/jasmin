...
class InterceptorPBTestCase(TestCase):
    def setUp(self, authentication = False):
        ...
        self.script_3_second = InterceptorScript(
            '_s=datetime.datetime.now()\nwhile (datetime.datetime.now()-_s).seconds<3: pass')
...
class RunScriptTestCases(IntercentorPBProxyTestCase):
    ...
    def test_slow_script_logging(self):
        ...
        last_msg = lc.records[len(lc.records) - 1].getMessage()
        self.assertIn('Execution delay [3s] for script [', last_msg)
