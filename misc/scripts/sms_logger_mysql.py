...
DB_NAME = os.getenv('DB_NAME', 'jasmin')
...
def db_connect():
    ...
    logger.info("Connected to MySQL (%s:%s/%s)", DB_HOST, DB_PORT, DB_NAME)
    return conn
...
def db_create_tables(conn):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS mt_messages (
            ...
            status_at        DATETIME,
            INDEX idx_mt_uid          (uid),
            INDEX idx_mt_routed_cid   (routed_cid),
            INDEX idx_mt_source_addr  (source_addr),
            INDEX idx_mt_dest_addr    (destination_addr),
            INDEX idx_mt_status       (status),
            INDEX idx_mt_status_at    (status_at)
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS mo_messages (
            ...
            INDEX idx_mo_cid          (cid),
            INDEX idx_mo_source_addr  (source_addr),
            INDEX idx_mo_dest_addr    (destination_addr),
            INDEX idx_mo_created_at   (created_at)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    """)
    cur.close()
    ...
    while True:
        ...
        try:
            ...
            if rk.startswith("submit.sm.") and not rk.startswith("submit.sm.resp."):
                ...
                cur = db.cursor()
                cur.execute(
                    """INSERT IGNORE INTO mt_messages
            ...
            # ── 4. deliver_sm → INSERT mo_messages ───────────────────
                ...
                cur = db.cursor()
                cur.execute(
                    """INSERT IGNORE INTO mo_messages
