...
class AmqpConfig(ConfigFile):
    ...
    def __init__(self, config_file=None):
        ...
        if CLOUDAMQP_URL is not None:
            ...
            m = re.search(r"^amqps?\:\/\/([^:]+)\:(.+)@((?!-)[-a-zA-Z0-9.]{1,63}(?<!-))\/(.+)$",
                          CLOUDAMQP_URL)
            if m is None:
                raise ValueError("Cannot parse CLOUDAMQP_URL: %s" % CLOUDAMQP_URL)
        ...
        # Persistence: when True, exchanges and queues are durable and
        # messages are published with delivery_mode=2 (persistent).
        self.persistence = self._getbool('amqp-broker', 'persistence', True)
