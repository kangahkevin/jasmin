...
class AmqpFactory(ClientFactory):
    ...
    def preConnect(self):
        ...
        if self.channelReady is None or (isinstance(self.channelReady, defer.Deferred) and self.channelReady.called):
    ...
    def exchange_declare(self, **keys):
        """Wrapper around channel's exchange_declare that applies persistence."""
        if self.config.persistence and 'durable' not in keys:
            keys['durable'] = True
        return self.chan.exchange_declare(**keys)
    ...
    def publish(self, **args):
        ...
        if self.config.persistence and 'content' in args:
            content = args['content']
            if hasattr(content, 'properties'):
                content.properties.setdefault('delivery-mode', 2)
