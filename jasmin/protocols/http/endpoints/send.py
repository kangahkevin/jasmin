...
class Send(Resource):
    ...
    def route_routable(self, updated_request):
        try:
            ...
            use_json = b'format' in updated_request.args and updated_request.args[b'format'][0] == b'json'

            # Default return
            if use_json:
                error_msg = response['return']
                if isinstance(error_msg, bytes):
                    error_msg = error_msg.decode()
                _return = json.dumps({'status': response['status'], 'error': str(error_msg)})
            else:
            ...
            if response['status'] == 200 and routedConnector is not None:
                ...
                if use_json:
                    _return = json.dumps({'status': 200, 'message_id': response['return']})
                else:
                    _return = 'Success "%s"' % response['return']

            if use_json:
                updated_request.responseHeaders.addRawHeader(b'content-type', b'application/json')
    ...
    def render_POST(self, request):
        ...
        try:
            ...
            fields = {b'to': {'optional': False, 'pattern': re.compile(rb'^\+{0,1}\d+$')},
                      ...
                      b'format'      : {'optional': True, 'pattern': re.compile(rb'^(json)$')}}
            ...
            if updated_request.args[b'dlr'][0] == b'yes':
                if b'dlr-level' not in updated_request.args:
                    ...
                    updated_request.args[b'dlr-level'] = [b'1']
            ...
            if use_json:
                ...
                if isinstance(error_msg, bytes):
                    error_msg = error_msg.decode()
                updated_request.responseHeaders.addRawHeader(b'content-type', b'application/json')
                return json.dumps({'status': response['status'], 'error': str(error_msg)}).encode()
            ...
            if use_json:
                updated_request.responseHeaders.addRawHeader(b'content-type', b'application/json')
                return json.dumps({'status': 500, 'error': str(response['return'])}).encode()
