...
from jasmin.tools.tlv_encoder import VALID_TLV_TYPES
...
SMPPClientConfigKeyMap = {
    'cid': 'id', 'host': 'host', 'port': 'port', 'bind_ip': 'bind_ip', 'username': 'username', 'logrotate': 'log_rotate',
...
SMPPClientConfigStringKeys = [
    'host', 'systemType', 'username', 'password', 'addressRange', 'useSSL', 'source_addr', 'custom_tlvs', 'bind_ip']

# When updating a key from RequireRestartKeys, the connector need restart for update to take effect
RequireRestartKeys = ['host', 'port', 'username', 'password', 'systemType', 'bind_ip']
...
def castOutputToBuiltInType(key, value):
    ...
    if key == 'custom_tlvs':
        ...
        for tlv in value:
            ...
            default_val = tlv.get('value')
            if default_val is not None:
                parts.append('0x%04X,%s,%s,%s,%s' % (tlv['tag'], tlv['type'], length_str, req, default_val))
            else:
...
def parseTlvString(value):
    """Parse connector-level TLV rules.

    Format (per entry; `;`-separated):
        tag,type,max_length[,required|optional[,default_value]]
    ...
    - `default_value` : optional. If set, this value is injected when the
                        per-message PDU does not carry this tag.
    """
    ...
    for entry in value.split(';'):
        ...
        if tlv_type not in VALID_TLV_TYPES:
            raise KeyError('Invalid TLV type: %s. Must be one of: %s' % (tlv_type, ', '.join(sorted(VALID_TLV_TYPES))))
...
class JCliSMPPClientConfig(SMPPClientConfig):
    ...
    def __init__(self, **kwargs):
        SMPPClientConfig.__init__(self, **kwargs)
    ...
    def log_update_requests_and_call(self, *args, **kwargs):
        ...
        if cmd == 'ok':
            ...
            try:
                ...
                configArgs = dict(self.sessBuffer)
...
class SmppCCManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['smppcm'].perspective_persist(opts.profile)
        if r:
    ...
    def _format_connector_row(self, connector):
        return "#%s %s %s %s %s" % (
    ...
    def _connector_list_header(self):
        return "#%s %s %s %s %s" % (
            'Connector'.ljust(35), 'Service'.ljust(7), 'Session'.ljust(16),
            'Starts'.ljust(6), 'Stops'.ljust(5))
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            ...
            for connector in connectors:
                if str(connector['id']) == arg:
                    found = connector
                    break
            if found is not None:
                self.protocol.sendData(self._connector_list_header(), prompt=False)
                self.protocol.sendData(self._format_connector_row(found), prompt=False)
                self.protocol.sendData(prompt=False)
                self.protocol.sendData('Connector [%s] found.' % arg)
            else:
                self.protocol.sendData('Connector [%s] not found.' % arg)
            return
        ...
        if (len(connectors)) > 0:
            ...
            for connector in connectors:
                counter += 1
                self.protocol.sendData(self._format_connector_row(connector), prompt=False)
                self.protocol.sendData(prompt=False)
