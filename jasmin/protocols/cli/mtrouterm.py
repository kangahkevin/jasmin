...
class MtRouterManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'mtroutes')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'mtroutes')
    ...
    def _mtroute_list_header(self):
        return "#%s %s %s %s %s" % (
            'Order'.ljust(5), 'Type'.ljust(23), 'Rate'.ljust(10),
            'Connectors'.ljust(48), 'Filters'.ljust(64))

    def _format_mtroute_row(self, order, mtroute):
        connectors = ''
        ...
        return "#%s %s %s %s %s" % (
            str(order).ljust(5), str(mtroute.__class__.__name__).ljust(23),
            rate.ljust(10), connectors.ljust(48), filters.ljust(64))
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            for e in mtroutes:
                order = list(e)[0]
                if str(order) == arg:
                    self.protocol.sendData(self._mtroute_list_header(), prompt=False)
                    self.protocol.sendData(self._format_mtroute_row(order, e[order]), prompt=False)
                    self.protocol.sendData(prompt=False)
                    self.protocol.sendData('MT Route [%s] found.' % arg)
                    return
            self.protocol.sendData('MT Route [%s] not found.' % arg)
            return
