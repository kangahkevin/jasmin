...
class HttpccManager(PersistableManager):
    ...
    def _httpcc_list_header(self):
        return "#%s %s %s %s" % (
            'Httpcc'.ljust(16), 'Type'.ljust(22), 'Method'.ljust(6), 'URL'.ljust(64))

    def _format_httpcc_row(self, cid, _httpcc):
        return "#%s %s %s %s" % (
            str(cid).ljust(16), str(_httpcc.__class__.__name__).ljust(22),
            _httpcc.method.upper().ljust(6), _httpcc.baseurl.ljust(64))

    def list(self, arg, opts):
        if arg != '':
            if arg in self.httpccs:
                self.protocol.sendData(self._httpcc_list_header(), prompt=False)
                self.protocol.sendData(self._format_httpcc_row(arg, self.httpccs[arg]), prompt=False)
                self.protocol.sendData(prompt=False)
                self.protocol.sendData('Httpcc [%s] found.' % arg)
            else:
                self.protocol.sendData('Httpcc [%s] not found.' % arg)
            return
        ...
        if (len(self.httpccs)) > 0:
            ...
            for cid, _httpcc in self.httpccs.items():
                counter += 1
                self.protocol.sendData(self._format_httpcc_row(cid, _httpcc), prompt=False)
                self.protocol.sendData(prompt=False)
