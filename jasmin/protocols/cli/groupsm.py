...
class GroupsManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'groups')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'groups')
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            found = None
            for group in groups:
                if str(group.gid) == arg:
                    found = group
                    break
            if found is not None:
                group_prefix = '!' if not found.enabled else ''
                self.protocol.sendData("#%s" % 'Group'.ljust(16), prompt=False)
                self.protocol.sendData("#%s" % (str(group_prefix + str(found.gid)).ljust(16)), prompt=False)
                self.protocol.sendData(prompt=False)
                self.protocol.sendData('Group [%s] found.' % arg)
            else:
                self.protocol.sendData('Group [%s] not found.' % arg)
            return
        ...
        if (len(groups)) > 0:
            ...
            for group in groups:
                counter += 1
                group_prefix = '!' if not group.enabled else ''
