...
class MoRouterManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'moroutes')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'moroutes')
    ...
    def _moroute_list_header(self):
        return "#%s %s %s %s" % (
            'Order'.ljust(5), 'Type'.ljust(23), 'Connectors'.ljust(48), 'Filters'.ljust(64))

    def _format_moroute_row(self, order, moroute):
        connectors = ''
        ...
        return "#%s %s %s %s" % (
            str(order).ljust(5), str(moroute.__class__.__name__).ljust(23),
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            for e in moroutes:
                order = list(e)[0]
                if str(order) == arg:
                    self.protocol.sendData(self._moroute_list_header(), prompt=False)
                    self.protocol.sendData(self._format_moroute_row(order, e[order]), prompt=False)
                    self.protocol.sendData(prompt=False)
                    self.protocol.sendData('MO Route [%s] found.' % arg)
                    return
            self.protocol.sendData('MO Route [%s] not found.' % arg)
            return
