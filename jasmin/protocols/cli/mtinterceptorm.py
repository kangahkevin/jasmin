...
class MtInterceptorManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'mtinterceptors')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'mtinterceptors')
    ...
    def _mtinterceptor_list_header(self):
        return "#%s %s %s %s" % (
            'Order'.ljust(5), 'Type'.ljust(20), 'Script'.ljust(47), 'Filters'.ljust(64))

    def _format_mtinterceptor_row(self, order, mtinterceptor):
        filters = ''
        ...
        return "#%s %s %s %s" % (
            str(order).ljust(5), str(mtinterceptor.__class__.__name__).ljust(20),
            repr(mtinterceptor.script).ljust(47), filters.ljust(64))
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            for e in mtinterceptors:
                order = list(e)[0]
                if str(order) == arg:
                    self.protocol.sendData(self._mtinterceptor_list_header(), prompt=False)
                    self.protocol.sendData(self._format_mtinterceptor_row(order, e[order]), prompt=False)
                    self.protocol.sendData(prompt=False)
                    self.protocol.sendData('MT Interceptor [%s] found.' % arg)
                    return
            self.protocol.sendData('MT Interceptor [%s] not found.' % arg)
            return
