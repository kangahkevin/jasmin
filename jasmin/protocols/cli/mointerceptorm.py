...
class MoInterceptorManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'mointerceptors')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'mointerceptors')
    ...
    def _mointerceptor_list_header(self):
        return "#%s %s %s %s" % (
            'Order'.ljust(5), 'Type'.ljust(20), 'Script'.ljust(47), 'Filters'.ljust(64))

    def _format_mointerceptor_row(self, order, mointerceptor):
        filters = ''
        ...
        return "#%s %s %s %s" % (
            str(order).ljust(5), str(mointerceptor.__class__.__name__).ljust(20),
            repr(mointerceptor.script).ljust(47), filters.ljust(64))
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            for e in mointerceptors:
                order = list(e)[0]
                if str(order) == arg:
                    self.protocol.sendData(self._mointerceptor_list_header(), prompt=False)
                    self.protocol.sendData(self._format_mointerceptor_row(order, e[order]), prompt=False)
                    self.protocol.sendData(prompt=False)
                    self.protocol.sendData('MO Interceptor [%s] found.' % arg)
                    return
            self.protocol.sendData('MO Interceptor [%s] not found.' % arg)
            return
