...
class UsersManager(PersistableManager):
    ...
    def persist(self, arg, opts):
        r = yield self.pb['router'].perspective_persist(opts.profile, 'users')
        if r:
    ...
    def load(self, arg, opts):
        r = yield self.pb['router'].perspective_load(opts.profile, 'users')
    ...
    def _format_user_row(self, user):
        user_prefix = '!' if not user.enabled else ''
        group = self.pb['router'].getGroup(user.group.gid)
        group_prefix = '!' if group is not None and not group.enabled else ''
    ...
    def list(self, arg, opts):
        ...
        if arg != '':
            ...
            if found is not None:
                self.protocol.sendData("#%s %s %s %s %s %s" % (
                    'User'.ljust(16), 'Group'.ljust(16), 'Username'.ljust(16),
                    'Balance'.ljust(7), 'MT'.ljust(6), 'Throughput'.ljust(8)), prompt=False)
                self.protocol.sendData(self._format_user_row(found), prompt=False)
                self.protocol.sendData(prompt=False)
                self.protocol.sendData('User [%s] found.' % arg)
            else:
                self.protocol.sendData('User [%s] not found.' % arg)
            return
        ...
        if (len(users)) > 0:
            ...
            for user in users:
                counter += 1
                self.protocol.sendData(self._format_user_row(user), prompt=False)
                self.protocol.sendData(prompt=False)
