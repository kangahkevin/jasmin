...
class FiltersManager(PersistableManager):
    ...
    def _filter_list_header(self):
        return "#%s %s %s %s" % (
            'Filter'.ljust(16), 'Type'.ljust(22), 'Routes'.ljust(6), 'Description'.ljust(32))

    def _format_filter_row(self, fid, _filter):
        routes = ''
        ...
        if _filter.__class__.__name__ in MTFILTERS:
            routes += 'MT'
        return "#%s %s %s %s" % (
            str(fid).ljust(16), str(_filter.__class__.__name__).ljust(22),
            routes.ljust(6), repr(_filter).ljust(32))

    def list(self, arg, opts):
        if arg != '':
            if arg in self.filters:
                self.protocol.sendData(self._filter_list_header(), prompt=False)
                self.protocol.sendData(self._format_filter_row(arg, self.filters[arg]), prompt=False)
                self.protocol.sendData(prompt=False)
                self.protocol.sendData('Filter [%s] found.' % arg)
            else:
                self.protocol.sendData('Filter [%s] not found.' % arg)
            return
        ...
        if (len(self.filters)) > 0:
            ...
            for fid, _filter in self.filters.items():
                counter += 1
                self.protocol.sendData(self._format_filter_row(fid, _filter), prompt=False)
                self.protocol.sendData(prompt=False)
