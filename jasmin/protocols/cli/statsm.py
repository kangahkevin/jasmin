...
class StatsManager(Manager):
    ...
    def users(self, arg, opts):
        headers = ["#User", "Binds", "Peers",
                   "SMPP-LA", "HTTP-Reqs", "HTTP-LA"]
    ...
    def smppcs(self, arg, opts):
        sc = SMPPClientStatsCollector()
        headers = ["#Connector", "Connected", "Bound", "Disconnected",
    ...
    def smppsapi(self, arg, opts):
        ...
        smpps_id = self.pb['smpps'].config.id
        headers = ["#Item", "Value"]

        table = []
        for k, v in sc.get(smpps_id).getStats().items():
            row = []
            row.append('#%s' % k)
            if k[-3:] == '_at':
                row.append(formatDateTime(v))
