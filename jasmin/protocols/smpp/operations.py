...
class SMPPOperationFactory:
    ...
    def _setConfigParamsInPDU(self, pdu, kwargs):
        ...
        # Connector-level TLV handling (default-value injection, validation,
        # type resolution) is performed in listeners.py at dispatch time.
    ...
    def isDeliveryReceipt(self, pdu):
        ...
        if 'short_message' in pdu.params:
            ...
            # Decode short_message for regex matching
            sm = pdu.params['short_message']
            if isinstance(sm, bytes):
                # Try UCS-2 first if the bytes look like UCS-2 (even length, starts with \x00)
                if len(sm) >= 2 and len(sm) % 2 == 0 and sm[0] == 0:
                    try:
                        sm_str = sm.decode('utf-16-be')
                    except (UnicodeDecodeError, ValueError):
                        sm_str = sm.decode('utf-8', 'ignore')
                else:
                    sm_str = sm.decode('utf-8', 'ignore')
            else:
                sm_str = sm
            ...
            # Clear text if it contains non-Latin characters (DLR text is informational only)
            if 'text' in ret and ret['text']:
                if re.search(r'[^\x00-\x7F]', ret['text']):
                    ret['text'] = ''
                else:
                    ret['text'] = re.sub(r'[^\x20-\x7E]', '', ret['text'])[:20]
        ...
        if ret['err'] != 'ND' and ret['err'].isdigit() and len(ret['err']) < 3:
    ...
    def getReceipt(self, dlr_pdu, msgid, source_addr, destination_addr, message_status, err, sub_date,
        ...
        # Pad sub/dlvrd/err to 3 chars per SMPP 3.4 Appendix B
        if isinstance(sub, bytes):
            sub = sub.decode()
        if isinstance(dlvrd, bytes):
            dlvrd = dlvrd.decode()
        if sub != 'ND' and sub.isdigit():
            sub = '{:0>3}'.format(sub)
        if dlvrd != 'ND' and dlvrd.isdigit():
            dlvrd = '{:0>3}'.format(dlvrd)
        err_str = str(err)
        if err_str.isdigit():
            err_str = '{:0>3}'.format(err_str)

        # Clear text if it contains non-Latin characters (DLR text is informational only)
        if isinstance(text, bytes):
            text = text.decode('ascii', 'ignore')
        text_str = str(text)
        if re.search(r'[^\x00-\x7F]', text_str):
            safe_text = ''
        else:
            safe_text = re.sub(r'[^\x20-\x7E]', '', text_str)[:20]

        # Build pdu
        if dlr_pdu == 'deliver_sm':
            short_message = r"id:%s sub:%s dlvrd:%s submit date:%s done date:%s stat:%s err:%s text:%s" % (
                msgid,
                sub,
                dlvrd,
