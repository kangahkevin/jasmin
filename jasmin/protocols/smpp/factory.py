...
class SMPPClientFactory(ClientFactory):
    ...
    def _connect(self):
        ...
        bind_ip = getattr(self.config, 'bind_ip', None)
        bind_addr = (bind_ip, 0) if bind_ip else None

        if self.config.useSSL:
            self.log.info('Establishing SSL connection to %s:%d%s', self.config.host, self.config.port,
                          ' from %s' % bind_ip if bind_ip else '')
            reactor.connectSSL(self.config.host, self.config.port, self, CtxFactory(self.config),
                               bindAddress=bind_addr)
        else:
            self.log.info('Establishing TCP connection to %s:%d%s', self.config.host, self.config.port,
                          ' from %s' % bind_ip if bind_ip else '')
...
class SMPPServerFactory(_SMPPServerFactory):
    ...
    def submit_sm_event_interceptor(self, system_id, *args):
        ...
        # Basic validation
        if SubmitSmPDU.params['destination_addr'] is None or len(SubmitSmPDU.params['destination_addr']) < 1:
        ...
        if interceptor is not None:
            ...
            d.addErrback(self.submit_sm_post_interception, system_id=system_id, proto=proto)
            return d
        else:
