...
class SMPPClientConfig:
    def __init__(self, **kwargs):
        ...
        self.bind_ip = kwargs.get('bind_ip', None)
        if self.bind_ip is not None and not isinstance(self.bind_ip, str):
            raise TypeMismatch('bind_ip must be a string or None')
        ...
        # Per-connector custom TLV rules.  Each entry is a dict:
        #   {'tag': int, 'type': str, 'length': int|None, 'required': bool,
        #    'value': str|None}
        # `length` = max allowed byte length (None = unlimited).
        # `required` = reject submits missing this tag.
        # `value` (optional) = default value injected when the per-message PDU
        #   does not carry this tag (backward-compat with pre-0.12 configs).
        ...
        for tlv in self.custom_tlvs:
            ...
            if 'type' not in tlv or tlv['type'] not in VALID_TLV_TYPES:
                raise UnknownValue('custom_tlv "type" must be one of: %s' % ', '.join(sorted(VALID_TLV_TYPES)))
