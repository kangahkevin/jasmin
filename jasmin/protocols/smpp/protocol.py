...
class SMPPClientProtocol(twistedSMPPClientProtocol):
    ...
    def endLongSubmitSmTransactionErr(self, failure):
        # Return on connection corruption (unrecoverable)
        if failure.check(SMPPClientConnectionCorruptedError):
            return
        return failure
...
class SMPPServerProtocol(twistedSMPPServerProtocol):
    ...
    def onPDURequest_unbind(self, reqPDU):
        was_bound = self.sessionState in [SMPPSessionStates.BOUND_RX,
                                          SMPPSessionStates.BOUND_TX,
                                          SMPPSessionStates.BOUND_TRX]
    ...
    def doBindRequest(self, reqPDU, sessionState):
        ...
        # Per-user IP whitelist check. Uses the same error code as auth
        # failure (ESME_RINVPASWD) so an attacker cannot distinguish
        # "valid creds, wrong IP" from "invalid creds".
        try:
            ...
            if smpps_cred is not None:
                networks = smpps_cred.ip_networks
                if networks and not is_ip_allowed(peer_host, networks=networks):
                    whitelist = smpps_cred.getAuthorization('ip')
                    self.log.warning(
            ...
            self.log.error('IP whitelist check failed for username "%s": %s — rejecting bind', username, e)
            self.sendErrorResponse(reqPDU, CommandStatus.ESME_RINVPASWD, username)
            return
