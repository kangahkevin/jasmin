...
class InterceptorPBConfig(ConfigFile):
    ...
    def __init__(self, config_file=None):
        ...
        # Redis (optional, exposed to interceptor scripts as 'rc')
        self.redis_enabled = self._getbool('interceptor', 'redis_enabled', False)
        self.redis_host = self._get('interceptor', 'redis_host', '127.0.0.1')
        self.redis_port = self._getint('interceptor', 'redis_port', 6379)
        self.redis_dbid = self._getint('interceptor', 'redis_dbid', 0)
        self.redis_password = self._get('interceptor', 'redis_password', None)
        self.redis_poolsize = self._getint('interceptor', 'redis_poolsize', 5)
