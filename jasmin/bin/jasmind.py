...
class JasminDaemon(BaseDaemon):
    ...
    def startHTTPApiService(self):
        ...
        smpps_id = 'smpps_01'
        if 'smpp-server-factory' in self.components:
            smpps_id = self.components['smpp-server-factory'].config.id
    ...
    def start(self):
        ...
        try:
            ...
            if 'rc' in self.components:
                self.components['router-pb-factory'].setRedisClient(self.components['rc'])
        except Exception as e:
