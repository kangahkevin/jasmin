...
class Thrower(Service):
    ...
    def addAmqpBroker(self, amqpBroker):
        ...
        yield self.amqpBroker.exchange_declare(exchange=self.exchangeName,
                                               type='topic')
...
class deliverSmThrower(Thrower):
    ...
    def __init__(self, config):
        ...
        self.stats = DeliverSmThrowerStatsCollector().get()
    ...
    def http_deliver_sm_callback(self, message):
        ...
        for dc in dcs:
            ...
            try:
                ...
                self.stats.inc('http_throw_error_count')
                ...
                if route_type == 'simple':
                    ...
                    if (str(e) not in noRetryErrors
                        ...
                        self.stats.inc('http_throw_retry_count')
                ...
                self.stats.inc('http_throw_count')
    ...
    def smpp_deliver_sm_callback(self, message):
        ...
        self.log.debug('Got one message (msgid:%s) to throw: %s', msgid, pdu)
        ...
        for dc in dcs:
            ...
            try:
                ...
                self.stats.inc('smpps_throw_error_count')
                ...
                if route_type == 'simple':
                    ...
                    if retry and self.getThrowingRetrials(message) < self.config.max_retries:
                        self.log.debug('Message try-count is %s [msgid:%s]: requeuing',
                                       self.getThrowingRetrials(message), msgid)
                        self.stats.inc('smpps_throw_retry_count')
                        yield self.rejectAndRequeueMessage(message)
                    elif retry:
                        self.log.warning('Message [msgid:%s] is no more processed after receiving "%s" error',
                                      msgid, str(e))
                ...
                # Everything is okay ? then:
                self.stats.inc('smpps_throw_count')
                yield self.ackMessage(message)
...
class DLRThrower(Thrower):
    ...
    def __init__(self, config):
        ...
        self.stats = DLRThrowerStatsCollector().get()
    ...
    def smpp_dlr_callback(self, message):
        ...
        sub = message.content.properties['headers'].get('sub', 'ND')
        dlvrd = message.content.properties['headers'].get('dlvrd', 'ND')
        text = message.content.properties['headers'].get('text', '')
        self.log.debug('Got one message (msgid:%s) to throw', msgid)
