...
from jasmin.store import get_backend
from jasmin.routing.stats import RouterStatsCollector
...
class RouterPB(pb.Avatar):
    def __init__(self, RouterPBConfig, persistenceTimer=True):
        ...
        # Stats
        self.stats = RouterStatsCollector().get()

        # Store backend (defaults to file; call setRedisClient to enable redis)
        self._store = None
        self._init_store()
        ...
        self.persistenceState = {'users': True, 'groups': True, 'moroutes': True, 'mtroutes': True,
                                 'mointerceptors': True, 'mtinterceptors': True}

        self.log.info('Router configured and ready.')

    def _init_store(self, redis_client=None):
        self._store = get_backend(self.config, redis_client)

    def setRedisClient(self, redis_client):
        if self.config.store_backend == 'redis':
            self._init_store(redis_client)
            self.log.info('Store backend switched to Redis')
    ...
    def addAmqpBroker(self, amqpBroker):
        ...
        yield self.amqpBroker.exchange_declare(exchange='messaging', type='topic')
        ...
        yield self.amqpBroker.exchange_declare(exchange='billing', type='topic')
    ...
    def persistenceTimerExpired(self):
        ...
        if has_updates:
            ...
            yield self.perspective_persist(scope='groups')
            yield self.perspective_persist(scope='users')
    ...
    def bill_request_submit_sm_resp_callback(self, message):
        ...
        if _user is None:
            self.stats.inc('billing_error_count')
            ...
            if _user.mt_credential.getQuota('balance') < amount:
                self.stats.inc('billing_reject_count')
                self.log.error(
                ...
                self.stats.inc('billing_charge_count')
                self.log.info('User [uid:%s] charged for amount: %s (bid:%s)', uid, amount, bid)
                yield self.ackMessage(message)
        else:
            self.stats.inc('billing_ack_count')
            self.log.info('User [uid:%s] has unlimited balance, acking billing request [bid:%s]', uid, bid)
            yield self.ackMessage(message)
    ...
    def perspective_persist(self, profile='jcli-prod', scope='all'):
        scopes = {
            'groups': ('router-groups', self.groups),
            'users': ('router-users', self.users),
            'moroutes': ('router-moroutes', self.mo_routing_table),
            'mtroutes': ('router-mtroutes', self.mt_routing_table),
            'mointerceptors': ('router-mointerceptors', self.mo_interception_table),
            'mtinterceptors': ('router-mtinterceptors', self.mt_interception_table),
        }

        try:
            for scope_name, (key, data) in scopes.items():
                if scope not in ['all', scope_name]:
                    continue
                if scope_name in ['groups', 'users'] and isinstance(data, list) and len(data) == 0:
                    self.log.warning('Refusing to persist empty %s — would destroy saved data', scope_name)
                    continue
                self.log.info('Persisting current %s configuration to [%s] profile',
                              scope_name.capitalize(), profile)
                yield self._store.save(key, data, profile)
                self.persistenceState[scope_name] = True
                if scope_name == 'users':
                    for u in self.users:
                        u.mt_credential.quotas_updated = False

        except IOError as e:
            self.log.error('Cannot persist: %s', e)
    ...
    def perspective_load(self, profile='jcli-prod', scope='all'):
        load_order = ['groups', 'users', 'mointerceptors', 'mtinterceptors', 'moroutes', 'mtroutes']
        keys = {
            'groups': 'router-groups',
            'users': 'router-users',
            'mointerceptors': 'router-mointerceptors',
            'mtinterceptors': 'router-mtinterceptors',
            'moroutes': 'router-moroutes',
            'mtroutes': 'router-mtroutes',
        }

        try:
            for scope_name in load_order:
                if scope not in ['all', scope_name]:
                    continue

                self.log.info('Loading/Activating [%s] profile %s configuration',
                              profile, scope_name)

                data = yield self._store.load(keys[scope_name], profile)
                if data is None:
                    self.log.warning('No data found for [%s] in profile [%s]', scope_name, profile)
                    continue
            ...
            self.log.error('Cannot load configuration: %s', e)
            defer.returnValue(False)
        except Exception as e:
    ...
    def perspective_user_authenticate(self, username, password):
        self.log.debug('Authenticating with username:%s', username)
    ...
    def perspective_group_remove(self, gid):
        ...
        for _group in self.groups:
            if gid == _group.gid:
                ...
                # Set persistence state to False (pending for persistence)
                self.persistenceState['groups'] = False
                self.persistenceState['users'] = False
                return True
