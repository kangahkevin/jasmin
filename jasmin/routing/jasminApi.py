...
class SmppsCredential(CredentialGeneric):
    ...
    # Default IP whitelist matches everything (IPv4 + IPv6) — no regression
    # when the attribute is absent on pickled users from older versions.
    DEFAULT_IP_WHITELIST = '0.0.0.0/0,::/0'
    ...
    def __init__(self, default_authorizations=True):
        ...
        #         Default is "any IPv4 + IPv6" so upgrading doesn't lock users out.
        self.authorizations = {
            'bind': default_authorizations,
            'ip': self.DEFAULT_IP_WHITELIST,
        }
        self._ip_networks = None
    ...
    def setAuthorization(self, key, value):
        ...
        if key == 'ip':
            from jasmin.tools.ipmatch import validate_whitelist, parse_networks
            ok, err = validate_whitelist(value)
            if not ok:
                raise jasminApiCredentialError(
                    'Authorization ip is not a valid value (%r): %s' % (value, err))
            self.authorizations[key] = value
            self._ip_networks = parse_networks(value)
            return
    ...
    def ip_networks(self):
        """Pre-parsed network objects for the IP whitelist (lazy-init for pickle compat)."""
        nets = getattr(self, '_ip_networks', None)
        if nets is None:
            from jasmin.tools.ipmatch import parse_networks
            nets = parse_networks(self.getAuthorization('ip'))
            self._ip_networks = nets
        return nets
