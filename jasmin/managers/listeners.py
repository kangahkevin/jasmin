...
def _log_tlvs(log, tlvs, prefix, identifier):
    """Log a list of TLV tuples with a consistent format."""
    for t in tlvs:
        if not t:
            log.info("TLV %s [%s] [tag:?] [type:unknown] [value_preview:?]",
                     prefix, identifier)
            continue
        try:
            tag_int = int(t[0]) & 0xFFFF
        except (ValueError, TypeError):
            tag_int = None
        tlv_type = t[2] if len(t) >= 3 else None
        val_preview = repr(t[3])[:60] if len(t) >= 4 else '?'
        if tag_int is not None:
            log.info(
                "TLV %s [%s] [tag:0x%04X] [type:%s] [value_preview:%s]",
                prefix, identifier, tag_int,
                tlv_type if tlv_type else 'unknown', val_preview)
        else:
            log.info(
                "TLV %s [%s] [tag:%s] [type:%s] [value_preview:%s]",
                prefix, identifier, t[0],
                tlv_type if tlv_type else 'unknown', val_preview)
...
class SMPPClientSMListener:
    ...
    def submit_sm_callback(self, message):
        ...
        try:
            ...
            _log_tlvs(self.log, pdu_tlvs_before, 'TX-pre', 'msgid:%s' % msgid)

            # Inject connector-level default TLV values for tags absent from the PDU
            existing_tags = {int(t[0]) & 0xFFFF for t in pdu_tlvs_before if t}
            for tlv_def in connector_tlvs:
                default_val = tlv_def.get('value')
                if default_val is not None and int(tlv_def['tag']) & 0xFFFF not in existing_tags:
                    pdu_tlvs_before.append(
                        (tlv_def['tag'], tlv_def.get('length'), tlv_def['type'], default_val))
            ...
            _log_tlvs(self.log, SubmitSmPDU.custom_tlvs, 'TX-post', 'msgid:%s' % msgid)
        ...
        try:
            ...
            if r.response.status == CommandStatus.ESME_ROK:
                ...
                if 'sar_msg_ref_num' in r.request.params:
                    splitMethod = 'sar'
                elif (UDHI_INDICATOR_SET
                      and len(r.request.params.get('short_message', b'')) >= 6
                      and r.request.params['short_message'][:3] == b'\x05\x00\x03'):
                    splitMethod = 'udh'
...
[from:%s] [to:%s] [content:%s] [tlvs:%s]",
    ...
    def concatDeliverSMs(self, HSetReturn, hashKey, splitMethod, total_segments, msg_ref_num, segment_seqnum):
        ...
        yield self.redisClient.expire(hashKey, self.config.long_deliver_sm_expiry)
    ...
    def code_dlr_msgid(self, pdu):
        ...
        try:
            ...
            ret = str(dlr_id).lower().lstrip('0') or '0' if dlr_id is not None else '0'
    ...
    def deliver_sm_event_interceptor(self, smpp, pdu):
        ...
        if interceptor is not None:
            ...
            d.addErrback(self.deliver_sm_event_post_interception, routable=routable, smpp=smpp)
            return d
        else:
    ...
    def deliver_sm_event_post_interception(self, *args, **kw):
        ...
        try:
            ...
            if inbound_tlvs:
                connector_tlvs = getattr(self.SMPPClientFactory.config, 'custom_tlvs', [])
                routable.pdu.custom_tlvs = resolve_tlv_types(inbound_tlvs, connector_tlvs)
                _log_tlvs(self.log, routable.pdu.custom_tlvs, 'RX',
                          'cid:%s' % self.SMPPClientFactory.config.id)
            ...
            if routable.pdu.dlr is None:
                ...
                elif (UDHI_INDICATOR_SET and not_class2
                      and message_content is not None and len(message_content) >= 6
                      and message_content[:3] == b'\x05\x00\x03'):
                    splitMethod = 'udh'
...
[content:%s]",
            ...
            # Do not log text for privacy reasons
            # Added in #691
            _mc = message_content if 'message_content' in dir() else None
            if self.config.log_privacy:
                logged_content = '** %s byte content **' % (len(_mc) if _mc is not None else 0)
            else:
                logged_content = '%r' % _mc

            if 'routable' in dir() and routable is not None:
                ...
                self.log.info("SMS-MO [cid:%s] [i-status:%s] [content:%s]",
                              self.SMPPClientFactory.config.id,
                              e.status,
                              logged_content)
