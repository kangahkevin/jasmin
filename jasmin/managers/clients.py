...
class SMPPClientManagerPB(pb.Avatar):
    def __init__(self, SMPPClientPBConfig):
        ...
        # Store backend (defaults to file; setRedisClient enables redis)
        self._store = get_backend(self.config)
    ...
    def addRedisClient(self, redisClient):
        ...
        if self.config.store_backend == 'redis':
            self._store = get_backend(self.config, redisClient)
    ...
    def perspective_persist(self, profile='jcli-prod'):
        self.log.info('Persisting current configuration to [%s] profile', profile)
        ...
        try:
            ...
            yield self._store.save('smppccs', connectors, profile)
            self.persisted = True
        except IOError as e:
    ...
    def perspective_load(self, profile='jcli-prod'):
        self.log.info('Loading/Activating [%s] profile configuration', profile)

        try:
            loadedConnectors = yield self._store.load('smppccs', profile)
            if loadedConnectors is None:
                self.log.warning('No smppccs data found for profile [%s]', profile)
                defer.returnValue(False)
            ...
            self.log.error('Cannot load configuration from %s: %s', profile, str(e))
            defer.returnValue(False)
    ...
    def perspective_connector_add(self, ClientConfig):
        ...
        # submit.sm queue only needed for connectors that can send (transceiver/transmitter)
        if c.bindOperation != 'receiver':
            ...
            self.log.info('Skipping submit.sm queue for receiver-only connector [%s]', c.id)
    ...
    def perspective_connector_start(self, cid):
        ...
        # Start the queue consumer only for connectors that can send
        if connector['config'].bindOperation != 'receiver':
            ...
            self.log.info('No submit_sm queue consumer for receiver-only connector [%s]', cid)
    ...
    def perspective_connector_stop(self, cid, delQueues=False):
        ...
        if delQueues and connector['config'].bindOperation != 'receiver':
