...
class SMPPClientPBConfig(ConfigFile):
    ...
    def __init__(self, config_file=None):
        ...
        self.store_backend = self._get('client-management', 'store_backend', 'file')
        self.store_redis_prefix = self._get('client-management', 'store_redis_prefix', 'jasmin:')
        self.store_file_backup = self._getbool('client-management', 'store_file_backup', True)
...
class SMPPClientSMListenerConfig(ConfigFile):
    ...
    def __init__(self, config_file=None):
        ...
        self.dlr_lookup_max_retries = self._getint(
            'sm-listener', 'dlr_lookup_max_retries', 2)

        self.long_deliver_sm_expiry = self._getint(
            'sm-listener', 'long_deliver_sm_expiry', 300)
