...
class DLR(Content):
    ...
    def __init__(self, pdu_type, msgid, status, smpp_msgid=None, cid=None, dlr_details=None):
        ...
        properties = {'message-id': str(msgid), 'headers': {'type': pdu_type.name,
                                                                 'status_at': str(datetime.datetime.now())}}
        ...
        if pdu_type == CommandId.submit_sm_resp and smpp_msgid is not None:
            ...
            if isinstance(smpp_msgid, bytes):
                smpp_msgid = smpp_msgid.decode()
            properties['headers']['smpp_msgid'] = smpp_msgid.lower().lstrip('0') or '0'
...
class SubmitSmContent(PDU):
    ...
    def __init__(self, uid, body, replyto, submit_sm_bill=None, priority=1, expiration=None, msgid=None,
        ...
        if priority < 0:
            raise InvalidParameterError("Priority must be a non-negative integer, it is actually set to %s" %
                                        priority)
