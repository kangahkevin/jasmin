...
VALID_TLV_TYPES = frozenset({'Int1', 'Int2', 'Int4', 'Int8', 'OctetString', 'COctetString'})
_VALID_TLV_TYPES = VALID_TLV_TYPES
...
def install_pdu_encoder_patch() -> None:
    ...
    def _encode_raw_with_int8(self, tlvs):
        ...
        for tag, length, _value_type, value in int8_entries:
            ...
            result += Int2Encoder().encode(tag) + Int2Encoder().encode(len(encoded_value)) + encoded_value
...
def install_sendpdu_wire_logger() -> None:
    ...
    def _send_with_wire_log(self, pdu):
        ...
        try:
            ...
            if written:
                # PDU already on the wire — only call onSMPPOperation via
                # the original path would re-send, so just run the bookkeeping.
                try:
                    self.onSMPPOperation()
                except Exception:
                    pass
            else:
                # Pre-write failure — safe to fall back to the original path.
